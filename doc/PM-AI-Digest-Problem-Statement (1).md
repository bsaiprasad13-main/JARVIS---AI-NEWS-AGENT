...
### 5.4 Summarization
- Use Groq (Llama-3) to generate, per item:
...
- Favor fewer, larger batched API calls (e.g., one call for the full day's digest) over many small calls, for cost efficiency and better relative ranking.
...
- **Reliability:** Failures (source API down, LLM call failure) should be logged, not silent
...
## 8. Future Considerations (explicitly deferred, not v1)
...
## 9. Appendix: Verified Implementation Details

### 9.1 Verified Source Endpoints (confirmed working as of Aug 29, 2026)
...
### 9.2 Per-Source Structural Assumptions (fetch/parsing behavior)
...
### 9.3 Special Case: The Rundown AI Feed Structure
...
### 9.4 Special Case: Hacker News Missing Description
...
### 9.5 Verification Thresholds (starting defaults — tune after observing real data)
| Source | Threshold |
|---|---|
